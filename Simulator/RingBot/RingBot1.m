classdef RingBot1 < handle
    ...
    methods
        ...
        function obj = RingBot1(link_vectors, joint_axes,prismatic, link_colors)
        ...
        function [joint_angles, link_extensions] = invKinNum(obj, X, R, initial_joint_angles, initial_link_extensions, max_iter)
            alpha = 0.05;
            link_number = length(obj.link_vectors);
            joint_angles = initial_joint_angles;
            link_extensions = initial_link_extensions;
            eul = rotm2eul(R, 'ZYZ');
            X_star = [X ; eul'];
            thresh_x = 0.05; %max this can be is 0.10, the ring radius
            for ii = 1:max_iter
                pose = obj.fwdKin_end(joint_angles, link_extensions);
                err = (pose - X_star);
        ...
        function [joint_angles, link_extensions] = invKin(obj,X, R)
            % Generates the inverse kinematics for Ringbot1
            %
            % INPUTS 
            % Pose: [x y z phi th psi] according to Euler ZYZ angles
            %
            % OUTPUTS
            % joint_angles: nx1 vector
            % link_extensions: nx1 vector
            
            H = [R X; 0 0 0 1];
            d = norm(obj.link_vectors{4}) + norm(obj.link_vectors{5}) + obj.ring_radius;
            p = H * [-d;0;0;1];
            th1 = atan2(p(2), p(1));
            th2 = -atan2(p(3) - norm(obj.link_vectors{1}), sqrt(p(2)^2 + p(1)^2));
            d3 = sqrt((p(2)^2 + p(1)^2)/(cos(th2)^2)) - norm(obj.link_vectors{2}) - norm(obj.link_vectors{3});
            
            R_set = threeD_rotation_set([th1 th2],{'z', 'y'});
            R_p = rotation_set_cumulative_product(R_set);

            R_w = R_p{end}.' *  R;
  
            th4 = atan2(R_w(3,1), R_w(2,1));
            th5 = atan2(sqrt(1-R_w(1,1)^2), R_w(1,1));
            th6 = atan2(R_w(1,3), -R_w(1,2)); 
            
            
            joint_angles = [th1;th2;th4;th5;th6];
            link_extensions = [0;d3;0;0;0];
            %{
