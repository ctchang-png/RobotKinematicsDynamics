function [] = play_trajectory(trajectory, use_velocity, simulator)
% SET SIMULATOR TO FALSE IF USING THE PHYSICAL ROBOT
% repeat_waypoints
%
...
for i = 1 : (size(trajectory, 2) - 1)
    ...
    if (use_velocity)
      cmd.velocity = trajectory_vel(:,i)' * frequency; % transpose turns column into row vector for commands
    end
    robot_hardware.set(cmd);
...
cmd.velocity = zeros(size(trajectory_vel(:,1)))';
robot_hardware.set(cmd);
%% --------------- END STUDENT SECTION ------------------------------------
