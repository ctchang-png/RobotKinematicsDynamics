...
for segment = 1:num_segments
    ...
    q0 = waypoints(:,segment);
    qf = waypoints(:,segment+1);
    t0 = times(segment);
    tf = times(segment+1);
    
    vmax = (qf - q0) / (tf-t0-ramp_time);
    ta = t0 + ramp_time;
    tb = tf - ramp_time;

    
    t = linspace(t0, ta, num_ramp_points);
    q_up = q0 + (vmax/(2*ramp_time)) .* (t - t0).^2;
    trajectory(:, segment_start_point : segment_start_point + num_ramp_points - 1) = q_up; 
    
    if (segment_end_point - num_ramp_points) - (segment_start_point + num_ramp_points) > 0
        t = linspace(ta, tb, points_in_segment - 2*num_ramp_points);
        q_fl = trajectory(:, segment_start_point + num_ramp_points - 1) + vmax*(t - ta);
        trajectory(:, segment_start_point + num_ramp_points : segment_end_point - num_ramp_points) = q_fl;
    end
    t = linspace(tb, tf, num_ramp_points);
    q_dn = trajectory(:,segment_end_point - num_ramp_points) + vmax*(t-tb) - (vmax/(2*ramp_time)) * (t - tb).^2;
                 % ramp up
    
    trajectory(:, segment_end_point - num_ramp_points + 1 : segment_end_point) = q_dn;
